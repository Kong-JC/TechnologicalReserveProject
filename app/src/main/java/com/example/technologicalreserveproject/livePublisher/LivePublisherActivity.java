package com.example.technologicalreserveproject.livePublisher;
...
public class LivePublisherActivity extends Activity {
    ...
    private final int FRAMERATE_DEF = 30;
    //	private final int BITRATE_DEF = 800 * 1000;
    private final int BITRATE_DEF = 480 * 640;

    private final int SAMPLE_RATE_DEF = 22050;
    private final int CHANNEL_NUMBER_DEF = 2;

    private final String LOG_TAG = "LivePublisherActivity";
//	private final boolean DEBUG_ENABLE = false;

    public static String _rtmpUrl = "rtmp://192.168.101.124:1935/live/stream";
        ...
        public void run() {
            while (!_h264EncoderThread.interrupted() && _bStartFlag) {
                ...
                if (iSize > 0) {
                    ...
                    if (yuvData == null) continue;


                    if (_bIsFront)
                        _yuvEdit = _swEncH264.YUV420pRotate270(yuvData, HEIGHT_DEF, WIDTH_DEF);
                    else _yuvEdit = _swEncH264.YUV420pRotate90(yuvData, HEIGHT_DEF, WIDTH_DEF);
...
//							}
    ...
    private Camera.PreviewCallback _previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] YUV, Camera currentCamera) {
            ...
            if (yuv420 == null) return;
            if (!_bStartFlag) return;

            _yuvQueueLock.lock();
            if (_YUVQueue.size() > 1) _YUVQueue.clear();
    ...
    public void InitCamera() {
        ...
        if (iNV21Flag != 0) _iCameraCodecType = iNV21Flag;
        else if (iYV12Flag != 0) _iCameraCodecType = iYV12Flag;
...
//		}
    ...
    private OnClickListener _switchCameraOnClickedEvent = new OnClickListener() {
        ...
        public void onClick(View arg0) {
            ...
            if (_bIsFront) _mCamera = Camera.open(Camera.CameraInfo.CAMERA_FACING_BACK);
            else _mCamera = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT);
...
//		RelativeLayout rCameraLayout = (RelativeLayout) findViewById(R.id.cameraRelative);
        ...
        _mSurfaceView = this.findViewById(R.id.surfaceViewEx);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _rtmpUrl = getIntent().getStringExtra("pubUrl");
    ...
    protected void onDestroy() {
        super.onDestroy();
        Log.i(LOG_TAG, "LivePublisherActivity onDestroy...");
    }
